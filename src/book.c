...
char ownbookfile[1024];
FILE *f_book = NULL;
bool ownbook = true;

bool using_book(void)
{
    return ownbook && f_book;
}
...
void set_ownbookfile( char * fname )
{
    strcpy(ownbookfile, fname);
    open_book();
}
...
#define MAX_BOOK_MOVES 100

bool check_book( char * fen, char * move )
{
    entry_t entry;
    uint64 key;
    entry_t entries[MAX_BOOK_MOVES];
    int count;
    int i, pos, sel;
    int offset;
    int weights;
    ...
    {
        key = hash_book(fen);
        offset = find_key(f_book, key, &entry);
        if(entry.key != key) {
            close_book();
            move[0] = 0;
            return false;
        ...
        entries[0]=entry;
        count=1;
        fseek(f_book, 16*(offset+1), SEEK_SET);
        while(1){
            pos = entry_from_file(f_book, &entry);
            if(pos || entry.key!=key || count==MAX_BOOK_MOVES){
                break;
            }
            entries[count++] = entry;
        }
        ...
        if( count > 1 )
        {
            ...
            // if(( rand() % 100 ) < 20)
            // {
                // for( i=0; i < count; i++) entries[i].weight=1;
            // }
            weights = 0;
            for( i=0; i < count; i++) weights += entries[i].weight;
            pos = rand() % weights;
            weights = 0;
            for( i=0; i < count; i++)
            {
                weights += entries[i].weight;
                if( pos <= weights )
                {
                    sel = i;
                    break;
        ...
        move_to_string(move, entries[sel].move);
        return true;
    }
    return false;
}
...
// {
    ...
    // ftime(&buffer);
    // srand((buffer.time * 1000) +buffer.millitm);
    // printf("%lld\n",(buffer.time * 1000) +buffer.millitm);
    // ownbook = 1;
    // open_book("irina.bin");

    // if( check_book( "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1", move ) )
        // {
            // printf(move);
        // }
    // close_book();
