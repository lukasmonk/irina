    ...
    for (k = desde; k < hasta && ok_time_kb; k++)
    {
        ...
        if (score > best_score)
        {
            ...
            if (score > alpha)
            {
                ...
                if (triangularLength[ply + 1])
                    triangularLength[ply] = triangularLength[ply + 1];
                else
                    triangularLength[ply] = 1;
            }
        }
    }
...
int repetitions()
{
    ...
    if (ilast < 0)
    {
        ...
        {
            if (historia[i] == hashkey)
                rep++;
        }
    }
    for (i = board.ply - 2; i >= ilast && i; i -= 2)
    {
        if (board.history[i].hashkey == hashkey)
            rep++;
    }
    return rep;
}
...
void quick_sort(int low, int high)
{
    ...
    if (low < high)
    {
        ...
        while (i < j)
        {
            while ((moveOrder[i].score <= moveOrder[pivot].score) && (i < high))
                i++;

            while (moveOrder[j].score > moveOrder[pivot].score)
                j--;
